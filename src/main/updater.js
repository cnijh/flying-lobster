...
async function checkForUpdate(force = false) {
  ...
  if (!force && now - lastCheck < CHECK_INTERVAL_MS) {
    ...
    if (cachedVersion) {
      ...
      console.log(
        `[Updater] Cached result: Current: ${currentVersion}, Latest: ${cachedVersion}, isNewer: ${isNewer}, isDismissed: ${isDismissed}, Update available: ${updateState.available}`,
      );
      notifyRenderer();
    }
    return updateState;
  ...
  try {
    ...
    console.log(
      `[Updater] Current: ${currentVersion}, Latest: ${latestVersion}, isNewer: ${isNewer}, isDismissed: ${isDismissed}, Update available: ${updateState.available}`,
    );
  } catch (err) {
...
async function startUpdate() {
  ...
  try {
    ...
    updateState.progress =
      "Installer opened! Drag to Applications to complete.";
    notifyRenderer();
