...
contextBridge.exposeInMainWorld("api", {
  ...
  // Agent switching
  onSwitchGateway: (cb) =>
    ipcRenderer.on("switch-gateway", (_event, gatewayId) => cb(gatewayId)),
  onCycleAgent: (cb) =>
    ipcRenderer.on("cycle-agent", (_event, direction) => cb(direction)),
  // Session management
  getSessions: (gatewayId) => ipcRenderer.invoke("get-sessions", gatewayId),
  refreshSessions: (gatewayId) =>
    ipcRenderer.invoke("refresh-sessions", gatewayId),
  openSession: (gatewayId, sessionData) =>
    ipcRenderer.invoke("open-session", gatewayId, sessionData),
  ...
  setGatewayVisibility: (gatewayId, visible) =>
    ipcRenderer.invoke("set-gateway-visibility", gatewayId, visible),
  // Auto-update support
  ...
  onUpdateStateChanged: (cb) =>
    ipcRenderer.on("update-state-changed", (_event, state) => cb(state)),
  // Screen Vision
