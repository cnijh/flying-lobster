...
function isTelemetryDisabled() {
  return process.env.FL_NO_TELEMETRY === 'true' || process.env.FL_NO_TELEMETRY === '1';
}
...
async function sendTelemetry() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('[Telemetry] Failed to send analytics:', response.status, response.statusText);
    }
  } catch (error) {
