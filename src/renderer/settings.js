  ...
  list
    ...
    .forEach((btn) =>
      btn.addEventListener("click", () =>
        startEdit(gateways.find((g) => g.id === btn.dataset.id)),
      ),
    );
  list
  ...
  list.querySelectorAll(".visibility-toggle").forEach((toggle) => {
    toggle.addEventListener("click", async () => {
      ...
      try {
        ...
        toggle.title = newVisible
          ? "Hide gateway from cycling"
          : "Show gateway from cycling";
      } catch (error) {
...
document.querySelectorAll(".shortcut-item[data-shortcut]").forEach((item) => {
  ...
  editIcon.addEventListener("click", () => {
    ...
    document
      .querySelectorAll(".shortcut-item.editing")
